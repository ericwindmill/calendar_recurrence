...
void main() {
  test('daily', () {
    ...
    final List<RecurrenceRule> rules = Recurrence.fromTexts(texts);
    expect(Recurrence.toTexts(rules), texts);
  });
  ...
  group('invalid cases', () {
    test('should raise exception when head of `RRULE:` is missing', () {
      Exception err;
      try {
        Recurrence.fromTexts(['FREQ=DAILY']);
      } catch (e) {
        err = e;
      }
    ...
    test('should raise exception when `FREQ` is missing', () {
      Exception err;
      try {
        Recurrence.fromTexts(['RRULE:COUNT=123']);
      } catch (e) {
        err = e;
      }
    ...
    test('should raise exception when `FREQ` appears multiple', () {
      Exception err;
      try {
        Recurrence.fromTexts(['RRULE:FREQ=DAILY;FREQ=WEEKLY']);
      } catch (e) {
        err = e;
      }
    ...
    test('should raise exception when `BYDAY` appears multiple', () {
      Exception err;
      try {
        Recurrence.fromTexts(['RRULE:FREQ=WEEKLY;BYDAY=TU;BYDAY=WE']);
      } catch (e) {
        err = e;
      }
    ...
    test('should raise exception when `COUNT` appears multiple', () {
      Exception err;
      try {
        Recurrence.fromTexts(['RRULE:FREQ=DAILY;COUNT=1;COUNT=2']);
      } catch (e) {
        err = e;
      }
    ...
    test('should raise exception when `INTERVAL` appears multiple', () {
      Exception err;
      try {
        Recurrence.fromTexts(['RRULE:FREQ=DAILY;INTERVAL=2;INTERVAL=3']);
      } catch (e) {
        err = e;
      }
